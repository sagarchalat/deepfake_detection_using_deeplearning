...
class VideoHandler(object):
    def __init__(self, video_path=0, img_path=None, args=None):
        ...
        self.writer = cv2.VideoWriter(args.save_path, cv2.VideoWriter_fourcc(*'H264'), self.video.get(cv2.CAP_PROP_FPS),
                                      (int(self.video.get(cv2.CAP_PROP_FRAME_WIDTH)), int(self.video.get(cv2.CAP_PROP_FRAME_HEIGHT))))
        if self.args.video_path == 0:
            self.writer1 = cv2.VideoWriter('static/videos/input.mp4', cv2.VideoWriter_fourcc(*'H264'), self.video.get(cv2.CAP_PROP_FPS),
                                          (int(self.video.get(cv2.CAP_PROP_FRAME_WIDTH)), int(self.video.get(cv2.CAP_PROP_FRAME_HEIGHT))))
